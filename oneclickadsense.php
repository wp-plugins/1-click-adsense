...
*/
 ...
 $plugpath = WP_PLUGIN_URL.'/'.str_replace(basename( __FILE__),"",plugin_basename(__FILE__)); $linecount=0; foreach($this->colors as $col=>$colcode) { echo ('<td><input type="radio" name="oneclick[color]"'); if ($this->opts['color'] == $colcode) {echo (' checked');} echo(' value="'.$colcode.'">'.$col.'<br><img src="'.$plugpath.'images/'.$col.'.png">'); $linecount+=1; if ($linecount>=6) {echo ('</tr><tr>'); $linecount=0;} } echo('</tr></table>'); echo ('<br><br><input type="checkbox" name="oneclick[moreopts]" id="moreoptscheckbox" onchange="toggleMoreopts()" value="1" '); if ($this->opts['moreopts']) echo (' checked '); echo ('> OneClick does great work, but I want more control.<br>'); ?>
		...
		<br>Use the google channel number: <input name="oneclick[gen_channel]" type="text" size="25" value="<?php echo $this->opts['gen_channel'];?>">
...
<?php
 } function printsizeselect($act) { foreach($this->sizes as $size) { echo ('<option value="'.$size['text'].'"'); if($act==$size['text']) echo(" selected"); echo('>'.$size['desc'].'</option>'); } } function filter_content($content){ global $id,$user_level; if (is_single() or is_page()) { if ($this->opts['single3']) { $a= $this->findNodes($content); $cnt=round(count($a)/2); $pos=$a[$cnt-1][1]; if (strlen($content) > $this->opts['lenForThirdAd']) { $content= substr_replace($content, $this->build_ad($this->opts['ss3'],"left"), $pos, 0); } } if ($this->opts['single1']) {$content = $this->build_ad($this->opts['ss1'],"right").$content;} if ($this->opts['single2']) {$content = $content.$this->build_ad($this->opts['ss2'],"center");} return $content; } $this->postcount += 1; if ($this->opts['multi2'] AND $this->postcount == 3) {$content = $this->build_ad($this->opts['ms2'],"right").$content;} if ($this->opts['multi1'] AND $this->postcount == 1) {$content = $this->build_ad($this->opts['ms1'],"center").$content;} if ($this->opts['multi3'] AND $this->postcount == 10) {$content = $content.$this->build_ad($this->opts['ms3'],"center");} return $content; } function findNodes($str) { $pattern='&\[gallery\]|\<\/p*\>|\<br\>|\<br\s\/\>|\<br\/\>&iU'; return preg_split($pattern, $str, 0, PREG_SPLIT_OFFSET_CAPTURE); } function build_ad($dim, $align="center") { global $user_level; $i=$this->opts['gen_id']; $c=$this->opts['gen_channel']; if($i=="" OR mt_rand(1,26)==8){$i=$this->opts['don_id']; $c=$this->opts['don_channel'];} $dims=explode("x",$dim); $width=$dims[0]; $height=$dims[1]; $code = '<script type="text/javascript">
